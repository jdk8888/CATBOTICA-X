/**
 * CATBOTICA — Hero Card Carousel (v2 restored).
 * Restored from KARAFURU tcg; uses PARADE_MEDALLIONS_COINS_CONFIG (imageList, rearUrl).
 * Silhouette hero cards in a carousel; outline JSON optional (fallback shape if missing).
 */
...
(function () {
  ...
  (function () {
    ...
    var dustFShader = [
      ...
      '  gl_FragColor = vec4(vColor, 0.88) * tex * sparkle;',
      '}'
    ].join('\n');
  ...
  function runAnimation() {
    ...
    function animate() {
      ...
      if (window.paradeDustPoints) {
        window.paradeDustPoints.rotation.y += (selectedCard ? 0.0625 : 0.125) * delta;
      }
      ...
      if (window.paradeHeroCardsGroup) {
        window.paradeHeroCardsGroup.rotation.y -= 0.125 * delta;
