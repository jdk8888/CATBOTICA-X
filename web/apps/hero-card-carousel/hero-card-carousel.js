/**
 * CATBOTICA — Hero Card Carousel (full controls).
 * Control panel, 12 medallions (three-layer parallax), inner sphere video, globe screens.
...
(function () {
  ...
  (function () {
    ...
    var scriptBase = (function () {
      var s = document.currentScript || document.querySelector('script[src*="hero-card-carousel.js"]');
  ...
  // Globe screens placeholder (5×7 grid); video textures can be wired when files are added
  (function () {
    var globeGroup = new THREE.Group();
    globeGroup.position.set(0, medallionCenterY, 0);
    var cols = 5, rows = 7;
    var cellW = 180, cellH = 120;
    var startX = -(cols - 1) * cellW * 0.5;
    var startZ = -(rows - 1) * cellH * 0.5;
    var geo = new THREE.PlaneGeometry(cellW * 0.9, cellH * 0.9);
    var mat = new THREE.MeshBasicMaterial({ color: 0x1a1a2e, side: THREE.DoubleSide });
    for (var r = 0; r < rows; r++) {
      for (var c = 0; c < cols; c++) {
        var plane = new THREE.Mesh(geo, mat.clone());
        plane.position.set(startX + c * cellW, 0, startZ + r * cellH);
  ...
  function runAnimation() {
    ...
    function animate() {
      ...
      if (window.paradeDustMaterial && window.paradeDustMaterial.uniforms.uTime) {
        ...
        if (window.paradeDustMaterial.uniforms.uLuminosity) window.paradeDustMaterial.uniforms.uLuminosity.value = typeof ctrl.particlesLum === 'number' ? ctrl.particlesLum : 1;
        if (window.paradeDustMaterial.uniforms.uColorTint) {
          var hex = (ctrl.particlesColor && ctrl.particlesColorHex) ? parseInt(ctrl.particlesColorHex.replace(/^#/, ''), 16) : 0xffffff;
          window.paradeDustMaterial.uniforms.uColorTint.value.setHex(hex);
        }
      }
      if (window.paradeDustPoints) {
        ...
        var spinSpeed = typeof ctrl.spinSpeed === 'number' ? ctrl.spinSpeed : 0.125;
      ...
      if (window.paradeRingMedallions) {
        ...
        var ms = typeof ctrlUI.medallionsScale === 'number' ? ctrlUI.medallionsScale : 1;
        window.paradeRingMedallions.scale.setScalar(ms);
        var mspin = typeof ctrlUI.medallionsSpinSpeed === 'number' ? ctrlUI.medallionsSpinSpeed : 0.4;
        window.paradeRingMedallions.rotation.y += mspin * delta;
      }
      if (window.paradeInnerSphere) window.paradeInnerSphere.visible = (ctrlUI.innerSphereVisible !== false);
      if (window.paradeGlobeScreens) window.paradeGlobeScreens.visible = (ctrlUI.globeScreensVisible === true);
